...
from models.Graph import Graph_Generator
from models.loss import Weighted_BCELoss
...
assert args.configs_path is not None, (
    "Argument --configs must be specified. Run 'python main.py --help' for help on arguments."
)
...
def main(args):
    if args.configs["Experiment"]["pretrain"]:
        ...
        args.optimizer_detection = torch.optim.Adam(
            params=args.detection_model.parameters(), lr=args.configs["Hyper"]["lr_detection"]
        )
        ...
        for epoch in range(1, epoch_num):
            ...
            print(f"Time Elapsed for epoch => {end_time - start_time}")
        ...
        if args.configs["Model"]["detection_model"] == "TCND":
            ...
            args.detection_model = FPN(n_classes=1)
            feature_dim = 128
        args.match_model = Graph_Generator(
            sinkhorn=args.configs["Model"]["Sinkhorn"], featuremap_dim=feature_dim, configs=args.configs
        )

        args.detection_loss_function = torch.nn.MSELoss()
        weight = (
            args.configs["Model"]["machL_weight"]
            if args.configs["Experiment"]["dataset_name"] in ["DeepGlobe", "Massachusetts"]
            else None
        )
        ...
        if args.configs["Model"]["Sinkhorn"]:
            args.optimizer_match = torch.optim.Adam(
                params=args.match_model.parameters(), lr=args.configs["Hyper"]["lr_match"]
            )
        else:
            args.optimizer_match = torch.optim.AdamW(
                params=args.match_model.parameters(), lr=args.configs["Hyper"]["lr_match"]
            )
