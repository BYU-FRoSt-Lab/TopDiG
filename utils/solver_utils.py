...
def compute_patch_boundingboxes(image_size, stride, patch_res):
    ...
    patch_boundingboxes = []
    for i in range(row_patch_count):
        ...
        for j in range(col_patch_count):
            ...
            patch_boundingbox = np.array(
                [row_slice_begin, col_slice_begin, row_slice_end, col_slice_end, i, j], dtype=np.int16
            )
            assert row_slice_end - row_slice_begin == col_slice_end - col_slice_begin == patch_res, (
                "ERROR: patch does not have the requested shape"
            )
    ...
    for i in range(new_m):
        for j in range(new_n):
            if i == new_m - 1 and j != new_n - 1:
                tmp_img = img[-patch_size:, j * stride_value : ((j + 1) * stride_value + double_bv), :]
            elif i != new_m - 1 and j == new_n - 1:
                tmp_img = img[i * stride_value : ((i + 1) * stride_value + double_bv), -patch_size:, :]
            elif i == new_m - 1 and j == new_n - 1:
            ...
            if i == 0 and j == 0:  # 左上角
                ...
                FullFeaturemap[
                    :, 0:most_value, boder_value + j * stride_value : boder_value + (j + 1) * stride_value
...
def predict_heatmap_WithOverlap_weight(args, img, feature_dim=64, patch_size=512, overlap_rate=1 / 4):
    ...
    patch_boundingboxes, row_patch_count, col_patch_count = compute_patch_boundingboxes(
        (height, width), stride=stride_value, patch_res=patch_size
    )
    ...
    for bbox in tqdm(patch_boundingboxes, desc="Running on patches", leave=True, disable=par_disable):
        ...
        with torch.no_grad():
            with autocast():
                ...
                vertices, vertices_score = getPoints(
                    tmp_heatmap, args.configs["Model"]["NUM_POINTS"], get_score=True, gap=args.gap
                )
        ...
        if len(tmp_polys_selected) > 0:
            ...
            tmp_polys_np = []
        instance = {"instances": tmp_polys_selected, "instances_np": tmp_polys_np, "region": bbox}
...
def predict_line_graph_WithOverlap_simple(args, img, patch_size=512, intersect_area=100):
    ...
    for bbox in tqdm(patch_boundingboxes, desc="Running on patches", leave=True, disable=par_disable):
        ...
        tmp_line_graphs, patch_nodes = get_lines(
            {"points_pred": vertices, "points_score": vertices_score, "PM_pred": PM_pred},
            thr=args.configs["Model"]["threshold"],
            score=0.6,
        )
    ...
    if len(all_patch_dicts):
        ...
        subgraphs = [
            FullDiGraph.subgraph(c) for c in sorted(nx.weakly_connected_components(FullDiGraph), key=len, reverse=True)
        ]
    else:
