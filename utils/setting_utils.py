...
def to_gpu(args, model, mode="DP"):
    if mode == "DP":
        ...
        model = nn.parallel.DistributedDataParallel(model.cuda(), device_ids=[args.gpu], find_unused_parameters=True)
    else:
        pass
...
def load_config(config_name="configs", config_dirpath="", try_default=False):
    ...
    try:
        ...
        if config_name == "configs" and config_dirpath == "":
            print("WARNING: the default configs file was not found....")
            return None
        if try_default:
            print(
                f"WARNING: configs file {config_filepath} was not found, opening default configs file configs.defaults.json instead."
            )
            return load_config()
        print(f"WARNING: configs file {config_filepath} was not found.")
        return None
...
def save_model(self, best_loss, current_loss, epoch):
    ...
    if self.configs["Experiment"]["detection_model_save"]:
        path_detection = os.path.join(
            self.model_dir,
            "points_epoch{}_valLoss_{:.5f}_IoU_{:.4f}_ptloss_{:.4f}.pkl".format(
                epoch, best_loss["total_loss"], best_loss["mask IoU"], best_loss["point_loss"]
            ),
        )
    ...
    if self.configs["Experiment"]["match_model_save"]:
        path_match = os.path.join(
            self.model_dir,
            "connection_epoch{}_valLoss_{:.5f}_IoU_{:.4f}_matchloss_{:.5f}.pkl".format(
                epoch, best_loss["total_loss"], best_loss["mask IoU"], best_loss["match_loss"]
            ),
        )
...
def save_pretrain_model(args, best_loss, epoch):
    ...
    if args.configs["Experiment"]["detection_model_save"]:
        path_detection = os.path.join(
            args.model_dir, "Pretrain_Epoch{}_valLoss_{:.4f}.pkl".format(epoch, best_loss["total_loss"])
        )
