...
from thop import clever_format, profile  # type: ignore
...
class TCND(BaseNet):
    ...
    def __init__(self, nclass, backbone, norm_layer=nn.BatchNorm2d, **kwargs):
        super().__init__(nclass, backbone, norm_layer=norm_layer, **kwargs)
        self.nclass = nclass
        ...
        self.side2 = nn.Sequential(
            nn.Conv2d(256, 1, 1, bias=True), norm_layer(1), nn.ConvTranspose2d(1, 1, 4, stride=2, padding=1, bias=False)
        )
        self.side3 = nn.Sequential(
            nn.Conv2d(512, 1, 1, bias=True), norm_layer(1), nn.ConvTranspose2d(1, 1, 8, stride=4, padding=2, bias=False)
        )
...
class LocationAdaptiveLearner(nn.Module):
    ...
    def __init__(self, nclass, in_channels, out_channels, norm_layer=nn.BatchNorm2d):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels, out_channels, 1, bias=True), norm_layer(out_channels), nn.ReLU(inplace=True)
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(out_channels, out_channels, 1, bias=True), norm_layer(out_channels), nn.ReLU(inplace=True)
        )
        self.conv3 = nn.Sequential(nn.Conv2d(out_channels, out_channels, 1, bias=True), norm_layer(out_channels))
    ...
    def forward(self, x):
        ...
        return x.view(x.size(0), self.nclass, -1, x.size(2), x.size(3))  # (N, n_class, 4, H, W)


def get_TCND(backbone="resnet50", pretrained=False, root="./pretrain_models", **kwargs):
    # infer number of classes
    return TCND(nclass=1, pretrained=pretrained, backbone=backbone, root=root, **kwargs)
