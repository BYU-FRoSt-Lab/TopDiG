...
def Train_TCND(epoch, train_step, args, scaler):
    ...
    for i, batch in enumerate(args.train_loader):
        ...
        if i == 0:
            ...
            visualize_keypoints(
                ...
                N=args.configs["Model"]["NUM_POINTS"],
                mode="train",
                env=args,
            )
        ...
        if i % 10 == 0:
            print(
                f"Epoch: {epoch} [{i + 1}/{len(args.train_loader)} ({100.0 * (i + 1) / len(args.train_loader):.2f}%)] | trainLoss:{loss.item():.6f}, node loss:{loss_node.item():.6f}, edge loss:{loss_edge.item():.6f}"
            )
    ...
    print(f"Train Epoch:{epoch} | Loss: {totalLosses.avg:.4f}")
...
def Val_TCND(epoch, val_step, best_loss, args):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(args.val_loader):
            ...
            if epoch + 1:
                vertices_pred, vertices_score = getPoints(
                    outputs[-1], args.configs["Model"]["NUM_POINTS"], get_score=True, gap=args.gap
                )
                points_gt = (
                    batch["skel_points"] if args.configs["Experiment"]["object_type"] == "line" else batch["polys"]
                )
            ...
            if i % 10 == 0:
                print(f"Evaluating [{i + 1}/{len(args.val_loader)} ({100.0 * (i + 1) / len(args.val_loader):.2f}%)]")
        ...
        print(
            f"Val Epoch:{epoch} | valLoss: {totalLosses.avg:.4f}, Node Loss: {Losses_node.avg:.4f}, Edge Loss: {Losses_edge.avg:.4f}"
        )
        # if epoch % 10 == 0 and epoch != 1:
        if epoch + 1:
            for key, value in recall.items():
                print(key, ":", f"{sum(value) / len(value):.4f}")
        #     for key, value in precision.items():
        #         print(key, ':', '{:.4f}'.format(sum(value) / len(value)))
        ...
        print(f"EarlyStopping counter: {args.early_stopping.counter} out of {args.early_stopping.patience}")
        if args.early_stopping.counter % 2 == 0 and args.early_stopping.counter > 0:
            print(
                "lr from {} to {}".format(
                    args.optimizer_detection.state_dict()["param_groups"][0]["lr"],
